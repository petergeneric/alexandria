...
2. **Second pass**: For each content file (`recoll-we-c-*.rclwe`):
   ...
   - Filter HTML through site-specific CSS selectors (`filter_html`)
   - Convert filtered HTML to Markdown via `htmd` (skipping script/style/nav/footer/header, stripping bold/italic)
   - Extract title from `<title>` tag
   - Extract domain from URL
   - Convert markdown to plaintext via `markdown_to_text`
   - Build a `PageSnapshot` with both raw HTML and plaintext
